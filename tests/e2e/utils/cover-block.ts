...
export class CoverBlockUtils {
	...
	async selectCoverBlock() {
		await this.editor.selectBlocks(
			this.editor.canvas.locator( '[data-type="core/cover"]' )
		);
	}
	...
	async addMediaToCover( mediaUrl?: string ) {
		...
		await this.page.evaluate( ( url ) => {
			const coverBlock = document.querySelector(
				'[data-type="core/cover"]'
			);
			if ( coverBlock ) {
				(
					coverBlock as HTMLElement
	...
	async openResponsiveFocalSettings() {
		...
		// Find responsive focal point settings panel
		const settingsPanel = this.page
			.locator( 'text=レスポンシブフォーカルポイント' )
			.or( this.page.locator( 'text=Responsive Focal Point' ) );
	...
	async addNewBreakpoint() {
		const addButton = this.page.locator(
			'button:has-text("Add New Breakpoint")'
		);
	 ...
	 */
	async setResponsiveFocalPoint(
		index: number,
		settings: ResponsiveFocalPoint
	) {
		// Select media type
		const mediaTypeSelect = this.page
			.locator( '.responsive-focal-row' )
			.nth( index )
			.locator( 'select' );
		await mediaTypeSelect.selectOption( settings.mediaType );

		// Input breakpoint
		const breakpointInput = this.page
			.locator( '.responsive-focal-row' )
			.nth( index )
		...
		const focalPointPicker = this.page
			.locator( '.responsive-focal-row' )
			.nth( index )
			.locator( '.components-focal-point-picker' );

		// Set focal point coordinates (click at relative position on picker)
		const pickerBounds = await focalPointPicker.boundingBox();
		if ( pickerBounds ) {
			const x =
				pickerBounds.x + ( pickerBounds.width * settings.x ) / 100;
			const y =
				pickerBounds.y + ( pickerBounds.height * settings.y ) / 100;
			await this.page.click( `css=.components-focal-point-picker`, {
				position: { x: x - pickerBounds.x, y: y - pickerBounds.y },
			} );
		}
	}
	...
	async verifyFrontendDisplay( expectedFocalPoints: ResponsiveFocalPoint[] ) {
		...
		try {
			...
			await expect(
				previewPage.locator( '.wp-block-cover' )
			).toBeVisible();

			// Verify data-fp-id attribute is set
			const coverElement = previewPage.locator(
				'.wp-block-cover[data-fp-id]'
			);
			...
			for ( const focalPoint of expectedFocalPoints ) {
				...
				const objectPosition = await coverElement.evaluate( ( el ) => {
					return window
						.getComputedStyle( el )
						.getPropertyValue( '--responsive-object-position' );
				} );
	 ...
	 * @param viewport.width  Viewport width
	 * @param viewport.height Viewport height
	 */
	async takeResponsiveScreenshot(
		...
		await expect( coverBlock ).toHaveScreenshot(
